...
import numpy as np
import pandas as pd
import statsmodels.api as sm  # kept for consistency / potential reuse
...
LOOKBACK_YEARS = 10  # SSA lookback (rolling 10-year window)

# Portfolio & costs
START_VALUE = 1000.0
TRADING_COST = 0.00086  # per side (buy OR sell)

# SSA structural parameters (Section 2.2.2 / 3.2.3)
SSA_L = 36  # embedding window length (months)
SSA_R = 12  # signal rank (number of eigentriples)
...
OUT_DIR_MC = Path().resolve() / "Monte_Carlo_Outputs" / "SSA_MC_classic_vs_riskadjusted"
...
# =============================== #
# 7) SSA FORECASTS & RANKINGS (CLASSIC VS RISK-ADJUSTED)
# =============================== #
...
def basic_ssa_forecast(log_returns: pd.Series,
                       ...
                       r: int = SSA_R) -> float:
    """
    Basic recurrent SSA one-step-ahead forecast for a univariate
    monthly log-return series, as in Section 2.2.2.

    - Uses window length L and signal rank r.
    - Handles degenerate cases via a simple AR(1) fallback.
    """
...
def ssa_signal_for_month(log_series: pd.Series,
                         forecast_month: pd.Timestamp,
                         lookback_years: int | None,
                         L: int = SSA_L,
                         r: int = SSA_R) -> float:
    """
    Compute SSA one-step-ahead forecast for a given contract and forecast month,
    ...
    if lookback_years is not None:
        ...
        s = s[s.index >= cutoff]
    # Require at least 12 observations for a meaningful seasonal window
    if s.size < 12:
        return np.nan
    return basic_ssa_forecast(s, L=L, r=r)


def build_rankings_from_log_rets(log_rets_sim: dict[str, pd.Series],
                                 lookback_years: int) -> tuple[dict, dict]:
    """
    Build monthly SSA-based rankings for LONG and SHORT directions based on
    (possibly transformed) log-return series.

    For each month in [FINAL_SIM_START, FINAL_END]:
      - For each contract, fit SSA to the last `lookback_years` of data
    ...
    while cur <= final_end:
        ...
        for t in tickers_sim:
            f = ssa_signal_for_month(log_rets_sim[t], cur, lookback_years,
                                     L=SSA_L, r=SSA_R)
            if not np.isfinite(f):
                continue
            rows.append({'ticker': t, 'signal': f})
...
# =============================== #
# 10) MONTE CARLO: CLASSIC VS RISK (SSA)
# =============================== #
def run_monte_carlo_both(n_runs=MC_RUNS, save_series=SAVE_SERIES, zero_noise=ZERO_NOISE):
    ...
    with redirect_stdout(tee):
        ...
        t0 = time.time()
        section(
            f"Starting Monte Carlo (MEB) — BOTH directions (classic vs risk-adjusted SSA) | "
        ...
        for run in range(1, n_runs + 1):
            ...
            # 3) SSA-based rankings (classic vs risk-adjusted)
        ...
        export_run_returns_and_plot(
            ...
            tracking_dir,
            "long_classic",
            f"SSA Monte Carlo | Long Classic | Cumulative value paths ({n_runs} runs)",
            "tab:green",
        )
        export_run_returns_and_plot(
            ...
            tracking_dir,
            "long_risk_adjusted",
            f"SSA Monte Carlo | Long Risk-adjusted | Cumulative value paths ({n_runs} runs)",
            "tab:blue",
        )
        export_run_returns_and_plot(
            ...
            tracking_dir,
            "short_classic",
            f"SSA Monte Carlo | Short Classic | Cumulative value paths ({n_runs} runs)",
            "tab:red",
        )
        export_run_returns_and_plot(
            ...
            tracking_dir,
            "short_risk_adjusted",
            f"SSA Monte Carlo | Short Risk-adjusted | Cumulative value paths ({n_runs} runs)",
            "tab:orange",
        )
        export_run_returns_and_plot(
            ...
            tracking_dir,
            "benchmark",
            f"SSA Monte Carlo | Equal-weight benchmark | Cumulative value paths ({n_runs} runs)",
            "tab:gray",
        )
